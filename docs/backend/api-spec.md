...
- Estado backend actual (2026-04-15): implementados `GET /health`, `GET /categories`, `GET /businesses`, `GET /businesses/{identifier}` y `GET /businesses/{identifier}/reviews`.
...
- Identificadores:
  - `business.slug` para rutas publicas de detalle (canonico).
  - `business.id` tambien se acepta temporalmente en lectura publica via `{identifier}` para compatibilidad.
...
- Visibilidad de datos en capa publica:
  - Solo negocios `published` en `GET /businesses` y `GET /businesses/{identifier}`.
  - Solo productos `active` en detalle de negocio.
  - Solo reseñas `published` en `GET /businesses/{identifier}/reviews`.
...
Nota:
- `identifier` acepta `slug` (canonico) o `id` UUID por compatibilidad.
...
## Reviews
### `GET /businesses/{identifier}/reviews`
...
- Bloque B1 publico implementado: `GET /categories`, `GET /businesses`, `GET /businesses/{identifier}`, `GET /businesses/{identifier}/reviews`.
